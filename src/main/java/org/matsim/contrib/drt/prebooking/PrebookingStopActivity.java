...
public class PrebookingStopActivity extends FirstLastSimStepDynActivity implements PassengerPickupActivity {
	...
	protected boolean isLastStep(double now) {
		// SINFONICA: Updated this logic, already underway in MATSim upstream
		boolean pickupDone = updatePickupRequests(now);
		boolean dropoffDone = processDropoffRequests(now);
		
		return pickupDone && dropoffDone && now >= endTime.get();
	}
